...
function compile() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        ts.sys.resolvePath = (path) => path_1.resolve(ts.sys.getCurrentDirectory(), path);
        {
            ...
            if (projectArgIndex != -1 && process.argv.length > projectArgIndex) {
                const folder = path_1.resolve(process.cwd(), process.argv[projectArgIndex + 1]);
        ...
        yield emitFile(cfg.fileNames[0], services, cfg);
    });
}
function watchFile(fileName, services, files, cfg) {
    if (!watchedFiles.has(fileName)) {
        watchedFiles.add(fileName);
        files[fileName] = { version: 0 };
        // Add a watch on the file to handle next change
        ts.sys.watchFile(fileName, (fileName, type) => {
...
function minify(files) {
    return __awaiter(this, void 0, void 0, function* () {
        const result = yield terser.minify(files, {
            ecma: 5,
            nameCache,
            mangle: {
                toplevel: false,
...
function emitFile(fileName, services, cfg) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        logErrors(services);
        const loadedLibs = [];
        function loadDclLib(lib) {
            const path = resolveFile(lib);
            if (path) {
                const json = JSON.parse(loadArtifact(lib));
                loadedLibs.push(...json);
                return true;
            }
            return false;
        }
        function loadJsLib(lib) {
            const path = resolveFile(lib);
            if (path) {
                const content = loadArtifact(lib);
                loadedLibs.push({
                    path: path_1.relative(ts.sys.getCurrentDirectory(), path),
                    content,
        ...
        for (let [, file] of out) {
            if (file.path.endsWith('.js') && file.content && !file.path.endsWith('.min.js')) {
                loadedLibs.push({
                    path: path_1.relative(ts.sys.getCurrentDirectory(), fileName),
                ...
                if (PRODUCTION || cfg.isDecentralandLib) {
                    ...
                    try {
                        const minificationResult = yield minify(loadedLibs.map(($) => $.content).join(';\n'));
        ...
        for (let [, file] of out) {
            ensureDirectoriesExist(path_1.dirname(file.path));
            if (file.content) {
...
function getConfiguration(packageJson, sceneJson) {
    const host = {
            ...
            if (packageJson.bundleDependencies instanceof Array) {
                packageJson.bundleDependencies.forEach(($, ix) => {
                    if (typeof $ == 'string') {
                        ...
                        console.error(`! Error: package.json .bundleDependencies must be an array of strings. The element number bundleDependencies[${ix}] is not a string.`);
                        hasError = true;
                    }
                });
            }
        ...
        if (resolved) {
            try {
                ...
                if (!libPackageJson.main) {
                    ...
                    main = path_1.resolve(path_1.dirname(resolved), libPackageJson.main);
                ...
                if (!libPackageJson.typings) {
                    ...
                    typings = path_1.resolve(path_1.dirname(resolved), libPackageJson.typings);
    ...
    if (libs.length && isDecentralandLib) {
        console.log('! Error: this project of type decentralandLibrary includes bundleDependencies. bundleDependencies are only allowed in scenes.');
        process.exit(1);
    }
    if (hasError) {
        console.log('tsconfig.json:');
        console.log(util_1.inspect(tsconfig, false, 10, true));
        process.exit(1);
    }
    ...
    function ensurePathsTopLevelNames(options, topLevelName) {
    ...
    if (hasCustomLibraries) {
        ...
        libs.forEach((lib) => {
            if (lib.name) {
                ...
                if (lib.typings) {
                    const relativePath = path_1.relative(path_1.dirname(tsconfigPath), lib.typings);
...
function printDiagnostic(diagnostic) {
    ...
    if (diagnostic.file) {
        ...
        console.log(`  Error ${diagnostic.file.fileName.replace(ts.sys.getCurrentDirectory(), '')} (${line + 1},${character + 1}): ${message}`);
    }
    else {
...
function validatePackageJsonForLibrary(packageJson, outFile) {
    if (!packageJson.main) {
        ...
        if (outFile !== mainFile) {
            const help = `(${outFile.replace(ts.sys.getCurrentDirectory(), '')} != ${mainFile.replace(ts.sys.getCurrentDirectory(), '')})`;
    ...
    if (!packageJson.typings) {
        ...
        if (resolvedTypings !== typingsFile) {
            const help = `(${resolvedTypings.replace(ts.sys.getCurrentDirectory(), '')} != ${typingsFile.replace(ts.sys.getCurrentDirectory(), '')})`;
            throw new Error(`! Error: package.json .typings does not match the emited file\n       ${help}`);
        }
    }
}
function validateSceneJson(sceneJson, outFile) {
    if (!sceneJson.main) {
